...
def main():
    import argparse
    p = argparse.ArgumentParser(description='Calculates global average of a model quantity. Writes to stdout.')
    ...
    p.add_argument("-t","--total",action='store_true',dest='totflag',\
                   default=False, help="If this flag is set, total is returned , not the average. ")
...
#    print("#Variable: ",var)
    ...
    for t in mObj.modelTimes:
        ...
        a = []
        for v in var:
            a.append(gitmLib.gblAverage(mObj, t, v,total=tflag))
